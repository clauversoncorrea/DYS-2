...
var query = "SELECT pd.data, COALESCE(pd.dest_xNome,cli.razao) dest_xNome, COALESCE(pd.dest_fone, cli.residencial, cli.residencial2) dest_fone, " +
    ...
    "pd.prazoEntrega, pd.seguro, IF (pd.tipo_movimentacao = 'compra',pd.freteCompra,pd.frete) frete,pd.fretePorConta, pd.orçamento, pt.imagem " +
    ...
    "where isnull(pp.ambiente_id) and pp._inativo = 0 and pd.id = " + id;
$.post(URL + "/jsonQuery/", g$.trataQuery(query)).then(function (data) {
    console.log(data.query);
...
var queryAmbientes = "select pd.data, pd.orçamento, pd.dest_xNome, COALESCE(pd.dest_fone, cli.residencial, cli.residencial2) dest_fone,COALESCE(pd.dest_cell, cli.celular, cli.celular2) dest_cell, pd.dest_cpf, dest_xCpl, pd.dest_xLgr, pd.dest_CEP,pd.dest_nro,pd.dest_xMun,pd.dest_UF, soli.fantasia as nomeSolic, " +
    ...
    "pp.desconto_percentual, IF (pd.tipo_movimentacao = 'compra',COALESCE(COALESCE(pp.preco_de_custo,pt.custo) * pp.quantidade,0),pp.total) total, pd.prazoEntrega, pd.seguro, IF (pd.tipo_movimentacao = 'compra',pd.freteCompra,pd.frete) frete,pd.fretePorConta, pd.orçamento,amb.nome as Ambiente, pt.imagem,amb.id as ambienteID " +
    ...
    "where not isnull(pp.ambiente_id) and pp._inativo = 0 and pd.id = " + id + " order by amb.nome";
