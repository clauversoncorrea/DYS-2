...
var query = "select pd.data, pd.dest_xNome, COALESCE(pd.dest_fone, cli.residencial, cli.residencial2) dest_fone,COALESCE(pd.dest_cell, cli.celular, cli.celular2) dest_cell, dest_cpf, dest_cnpj, dest_xCpl, pd.dest_xLgr, pd.dest_CEP,pd.dest_nro,pd.dest_xMun,pd.dest_UF, soli.razao as nomeSolic, " +
    ...
    "pp.desconto_percentual, IF (pd.tipo_movimentacao = 'compra',COALESCE(COALESCE(pp.preco_de_custo,pt.custo) * pp.quantidade,0),pp.total) total, pd.prazoEntrega, pd.seguro, IF (pd.tipo_movimentacao = 'compra',pd.freteCompra,pd.frete) frete, pd.orçamento, pt.imagem " +
