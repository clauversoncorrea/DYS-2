app.controller("tabela", function ($scope, $http, $compile, $rootScope) {
    ...
    g$._initTabela = function (elm, filtro, nome_procedure, idFuncao, isTela) {
        ...
        for (var i = 0; i < elm.tBodies[0].rows[0].cells.length; i++) {
            if (elm.tBodies[0].rows[0].cells[i].id == "td" && g$.filterCampo(elm.tBodies[0].rows[0].cells[i].dataset.le_do_campo).toUpperCase().trim() == "PAI")
    ...
    $scope.get = function (elm, template, nome, filtro, nome_procedure, idFuncao, isTela) {
        var keys, td, elm = elm, nome_proc_parametro, obj, select, campos, j = 0,
            nomeProc = 'node.le(' + elm.dataset.consulta_id + ',"' + $rootScope.user.banco + '", "' + filtro + '")';

        nomeProc = nomeProc.replace(/\//g, "½");
        ...
        if (nome_procedure && nome_procedure != "") {
            ...
            obj = { query: nomeProc };
            $http.post(URL + "/jsonQuery/", obj).success(function (response) {
                ...
                $scope[nome + "s"] = (select) ? response.data : response.data[0];
    ...
    $scope.dataTable = function (elm) {
        $http.get("/").success(function () {
            ...
            $("#view [data-id=" + elm.dataset.id + "]").DataTable({
                ...
                "iDisplayLength": (elm.dataset.tbl_resultados_por_pagina && elm.dataset.tbl_resultados_por_pagina != "null" && elm.dataset.tbl_resultados_por_pagina != "") ? elm.dataset.tbl_resultados_por_pagina : 10,
            ...
            if (elm.dataset.tbl_treeview == "1") $scope.init_treeview(elm);
        });
    }
    ...
    */
    ...
    g$.insereDadosEndereco = function (objRow, grava_tabela, trs, elm) {
        // update
        $http.put(URL + "/put/" + grava_tabela + "/", objRow)
            .then(function (data) {
                data = data.data;
                ...
                //refresh
                var alias = g$.filterTabela(trs[i].cells[1].dataset.le_da_tabela, false),
                    campo = g$.filterCampo(trs[i].children[1].dataset.le_do_campo),
                    filtro = alias + "." + campo + " = " + id,
                    nomeProc = 'node.le(' + elm.dataset.consulta_id + ',"' + $rootScope.user.banco + '", "' + filtro + '")';
                ...
                $http.get(URL + "/proc/" + nomeProc).success(function (response) {
                    ...
                    $http.get(URL + "/get/" + consulta).success(function (response) {
                        ...
                        g$.refreshRow(elm, trs[i], response.data);
                    });
                });
            });
    }

    g$.alteraCEP = function (elm) {
        var trs, cells, campo;
        $http.get("/").success(function () {
            ...
            trs.forEach(function (v, i) {
                // verificaCEP
                var _idObj = (trs[i].cells[1].querySelectorAll("span")[0]) ? trs[i].cells[1].querySelectorAll("span")[trs[i].cells[1].querySelectorAll("span").length - 1].innerHTML : trs[i].cells[1].innerHTML,
                    cep = trs[i].querySelector("td[data-nome='tbl_cep']"),
                    municipio = trs[i].querySelector("td[data-nome='cod_municipio']"),
                    bairro = trs[i].querySelector("td[data-nome='bairro']"),
                    cidade = trs[i].querySelector("td[data-nome='cidade']"),
                    uf = trs[i].querySelector("td[data-nome='uf']"),
                    objRow = {},
                    objCep = [], grava_na_tabela = elm.dataset.grava_tabela,
                    etapas = 0, fim = 2;

                objRow.id = _idObj;

                if (municipio && municipio.innerHTML.trim() == "") objCep[0] = 'cod_municipio';
                if (bairro && bairro.innerHTML.trim() == "") objCep[1] = 'bairro';
                if (cidade && cidade.innerHTML.trim() == "") objCep[2] = 'cidade';
                if (uf && uf.innerHTML.trim() == "") objCep[3] = 'uf';

                if (cidade && cidade.innerHTML.trim() != "" && uf && uf.innerHTML.trim() != "" && municipio && municipio.innerHTML.trim() == "") {
                    var queryPegaIBGE = "SELECT ibge FROM node.ibge WHERE Uf LIKE '" + uf.innerHTML.trim() + "' AND cidade LIKE '" + cidade.innerHTML.trim() + "' LIMIT 1";
                    $http.get(URL + "/get/" + queryPegaIBGE).success(function (data) {
                        // Trata Excecao
                        if (g$.exceptionRequisicao("IBGE - Tabela", data)) return;
                        if (data.data.ibge[0] && data.data.ibge[0] != "") {
                            objRow[g$.filterCampo(municipio.dataset.grava_no_campo, true)] = data.data.ibge[0];
                        }
                        etapas = etapas + 1;
                        if (etapas = fim) g$.insereDadosEndereco(objRow, grava_tabela, trs, elm);
                    });

                } else etapas = etapas + 1;

                if (cep && cep.innerHTML.trim() != "" && cep.innerHTML.length == 8) {

                    // cep ¦ endereco ¦ complemento ¦ bairro ¦ cidade ¦ uf ¦ unidade ¦ ibge ¦ gia
                    $http.get("https://viacep.com.br/ws/" + cep.innerHTML.trim() + "/json/unicode/").success(function (data) {
                        if (data.erro || data.status == 0) return console.log("CEP não encontrado!");
                        if (objCep.length) {
                            if (objCep.indexOf('cod_municipio') && !objRow[g$.filterCampo(municipio.dataset.grava_no_campo, true)]) objRow[g$.filterCampo(municipio.dataset.grava_no_campo, true)] = data.cep.replace("-", "");
                            if (objCep.indexOf('bairro')) objRow[g$.filterCampo(bairro.dataset.grava_no_campo, true)] = data.bairro;
                            if (objCep.indexOf('cidade')) objRow[g$.filterCampo(cidade.dataset.grava_no_campo, true)] = data.localidade;
                            if (objCep.indexOf('uf')) objRow[g$.filterCampo(uf.dataset.grava_no_campo, true)] = data.uf;
                    ...
                    if (i == trs.length - 1) $scope.setAttributesCells(elm);
                }
            });
        });
    }
            ...
            for (var i = 0; i < trs.length; i++) {
                ...
                for (var j = 0; j < cells.length; j++) {
                    ...
                    if (cells[j].dataset.nome && cells[j].dataset.nome != "null" && cells[j].dataset.nome.indexOf("|") > -1) {
                        var params = cells[j].dataset.nome.split("|"),
                            elemento = params[0].trim(),
                            prop = params[1].trim(),
                            valorprop = params[2].trim(),
                            filtro = params[3].trim();

                        filtro = (cells[j].querySelectorAll("span")[1]) ? filtro.replace("valor", cells[j].querySelectorAll("span")[1].innerHTML) : filtro.replace("valor", cells[j].innerHTML);

                        filtro = g$.validaCondicao(filtro);

                        if (filtro) {
                            if (prop == "display") cells[j].parentElement.querySelector("[data-id='" + elemento + "']").style.display = valorprop;
                            else if (prop == "nome" && $("[data-id='" + elemento + "']")[0].id == "icone") {
                                cells[j].parentElement.querySelector("[data-id='" + elemento + "']").className = "fa " + valorprop;
                            }
                        }
    ...
    colocaCombo = function (elm) {
        ...
        campo = parseInt(obj.combo_campo) ? g$.filterCampo(obj.combo_campo) : obj.combo_campo;
