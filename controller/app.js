...
g$.tiraVirgulaEntreAspas = function (query) {
    var res = query, simples = -1, dupla = -1, campos, inicio, fim, entreAspas, parou = 0, queryFinal = '', char;
    i = 0;
    // for (var i = 0; i < res.length; i++) {
    //     char = res.substring(i, i + 1);
    ...
    // }
    while (parou != res.length) {
        inicio = res.indexOf("'", parou);
        inicio = ((res.indexOf('"', parou) < inicio && res.indexOf('"', parou) > -1) || (inicio == -1 && res.indexOf('"', parou) > -1)) ? res.indexOf('"', parou) : inicio;
        if (inicio == -1) {
            queryFinal = queryFinal + res.substring(parou);
            parou = res.length;
        }
        else {
            char = res.slice(inicio, inicio + 1);
            fim = res.indexOf(char, inicio + 1);
            entreAspas = res.substring(inicio + 1, fim);
            queryFinal = queryFinal + res.substring(parou, inicio) + char + entreAspas.replace(/\,/g, "❜").replace(/\ /g, "‿").replace(/\=/g, "˭") + char;
            parou = fim + 1
        }
    }
...
addEventListener("keydown", function (e) {
    ...
    if ((event.ctrlKey && event.altKey && event.keyCode == "76") || (event.altKey && event.keyCode == "76")) {
...
g$.closeModalCust = function (id) {
    ...
    if (id == "modal-comboDYS") {
        g$.arrayTelas = g$.arrayTelas.filter(v => v.toString() != "ModalCombo");
        var elm = $("[data-id='" + g$.last_element + "']")[0];
        $("[data-tela='" + elm.dataset.menu_id + "'] .card-content")[0].focus();
        delete g$.last_element;
...
g$.omitirPropriedade = function (obj, insert) {
    var keys = Object.keys(obj), elm;
    for (var i = 0; i < keys.length; i++) {
        if ((obj[keys[i]] === null || obj[keys[i]] === 'null' || obj[keys[i]] === '') && insert) delete obj[keys[i]];
        if ((obj[keys[i]] === null || obj[keys[i]] === 'null' || obj[keys[i]] === '') && !insert) obj[keys[i]] = null;
    }
    return obj;
}
